...
if [ $? -eq 0 ]; then
	...
	then
       ...
       -jar target/authenticator-1.0.jar --server.port=8085 1>>build.logs 2>>build.logs &
	else
		stop_old_server
		echo "starting server "
		java -Xmx1024M -Xms1024M -jar target/authenticator-1.0.jar --server.port=8085 1>>build.logs 2>>build.logs &
	fi
	echo $! >> process.pid
else
